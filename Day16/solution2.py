hex_bin = {
    "0" : "0000",
    "1" : "0001",
    "2" : "0010",
    "3" : "0011",
    "4" : "0100",
    "5" : "0101",
    "6" : "0110",
    "7" : "0111",
    "8" : "1000",
    "9" : "1001",
    "A" : "1010",
    "B" : "1011",
    "C" : "1100",
    "D" : "1101",
    "E" : "1110",
    "F" : "1111"
}

input_hex = ""
input_bin = "" 
for item in input_hex:
    input_bin += hex_bin[item]
print(input_bin)

packet_versions = []


def subchecker(mode, lst):
    if mode == 0:
        return sum(lst)
    if mode == 1:
        totaliser = 1
        for item in lst:
            totaliser = totaliser * item
        return totaliser
    if mode == 2:
        return min(lst)
    if mode == 3:
        return max(lst)
    if mode == 5:
        if lst[0] > lst[1]:
            return 1
        return 0
    if mode == 6:
        if lst[0] < lst[1]:
            return 1
        return 0
    if mode == 7:
        if lst[0] == lst[1]:
            return 1
        return 0


def RunCode(start):
    packet_version = int(input_bin[start:start+3],2)
    packet_id = int(input_bin[start+3:start+6],2)
    packet_versions.append(packet_version)
    these_literals = []
    if packet_id == 4:
        literal = ''
        modified_literal = ''
        counter = 0
        while True:
            if input_bin[start+6+counter] == '0':
                literal += input_bin[start+6+counter:start+6+counter+5]
                counter += 5
                break
            else:
                literal += input_bin[start+6+counter:start+6+counter+5]
                counter += 5
        for i in range(0, len(literal),5):
            modified_literal += literal[i+1:i+5]
        return[counter+6,int(modified_literal,2)]
    else:
        length_id = input_bin[start+6]
        if length_id == '0':
            number_of_bits = int(input_bin[start+7:start+7+15],2)
            counter = 0
            while counter < number_of_bits:
                [counter1, literal] = RunCode(start+7+15+counter)
                counter += counter1
                these_literals.append(literal)
            return[7+15+counter,subchecker(packet_id,these_literals)]
        else:
            number_of_subpackets = int(input_bin[start+7:start+7+11],2)
            counter = 0
            num_of_subs = 0
            while num_of_subs < number_of_subpackets:
                [counter1, literal] = RunCode(start+7+11+counter)
                counter += counter1
                num_of_subs += 1
                these_literals.append(literal)
            return[7+11+counter,subchecker(packet_id,these_literals)]
    return 0

print(RunCode(0))


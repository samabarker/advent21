...
def checker(number_list, bingo_cards, counter = 0):
    for num in number_list:
        ...
        for i in range(0, len(bingo_cards)):
            if row_checker(bingo_cards[i], number_list, counter):
                return [row_checker(bingo_cards[i], number_list, counter),num, counter]
            if column_checker(bingo_cards[i], number_list, counter):
                return [column_checker(bingo_cards[i], number_list, counter), num, counter]


#Answer
def answer(matching_card, number_list, last_number_called):
    summer = 0
    for i in range(0,5):
        for j in range(0,5):
            if matching_card[i][j] not in number_list:
                summer += matching_card[i][j]
    return(summer * last_number_called)
...
last_number_called = checker(number_list, bingo_cards)[1]
trunc_list = number_list[0:checker(number_list, bingo_cards)[2]]

print(answer(matching_card, trunc_list, last_number_called))